...
function App() {
    ...
    async function hydratePasswords(newKey: CryptoKey) {
        setKey(newKey);
        await wait(500);
        const encryptedPasswords = JSON.parse(localStorage.getItem(PASSWORDS_STORAGE_KEY) || '');
    ...
    useEffect(() => {
        const rawCryptoKey = localStorage.getItem(CRYPTO_KEY_STORAGE_KEY);
    ...
    useEffect(() => {
        async function sync() {
            ...
            localStorage.setItem(PASSWORDS_STORAGE_KEY, JSON.stringify(encryptedPasswords));
        }
        sync();
    }, [decryptedPasswords]);

    function handleLogout() {
        localStorage.removeItem(CRYPTO_KEY_STORAGE_KEY);
        setKey(null);
    }
